...
static int kvm_vmi_create_view(struct kvm *kvm, struct kvm_vmi_view *uview)
{
	...
	view = kzalloc_obj(*view, GFP_KERNEL);
	if (!view)
		return -ENOMEM;
...
static int kvm_vmi_change_gfn(struct kvm *kvm, struct kvm_vmi_change_gfn *change)
{
	 ...
	 * races the free -> use-after-free in the arch invalidate path
	 * (KASAN slab-use-after-free in the arch stage-2 unmap path). gfn_to_memslot()/__kvm_faultin_pfn() require
