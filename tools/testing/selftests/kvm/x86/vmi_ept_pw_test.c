 ...
 * Uses vm_get_pte() to get the HVA of the PTE, then converts back to GPA.
 */
static uint64_t get_pt_page_gfn(struct kvm_vm *vm, uint64_t gva)
{
	...
	pte_hva = vm_get_pte(vm, gva);
	TEST_ASSERT(pte_hva != NULL, "vm_get_pte returned NULL for GVA 0x%lx",
		    (unsigned long)gva);
