 ...
 * kvm_vmi_view_force_pte_gfn() (arch/arm64/kvm/vmi.c) does
 *
 ...
 *
 * and the sibling helpers (clamp_view_prot / view_remap / view_denies /
 * autostep_mask) do the same. These run from the abort handler under
 * srcu_read_lock(&kvm->srcu) (kvm_handle_guest_abort, arm64/kvm/mmu.c:2125),
...
static void test_view_rcu_free_uaf(unsigned long nr_rounds)
{
	...
	*prot_hva = 0xC0FFEEULL;
	...
	for (i = 0; i < nr_rounds && !targ.done; i++) {
		 ...
		 * kvm_vmi_view_force_pte() returns false and the fault reaches
