 ...
 *
 * kvm_vmi_change_gfn() looks up the target view with xa_load(&vmi->views, id)
 * WITHOUT holding vmi->lock, then dereferences the returned kvm_vmi_view_data
 * (its gfn_overrides / gfn_override_pages xarrays, arch root) across many
 * statements. kvm_vmi_destroy_view() erases the view under vmi->lock and then
 * kfree()s it AFTER dropping the lock. The VMI ioctl handler (kvm_vmi_ioctl)
 * takes no lock and is .unlocked_ioctl, so two userspace threads on the same
 * vmi_fd can run CHANGE_GFN(view) and DESTROY_VIEW(view) concurrently. If the
 * change_gfn lookup wins the xa_load just before destroy_view's xa_erase, the
 * change_gfn then dereferences a freed view -> use-after-free.
 *
