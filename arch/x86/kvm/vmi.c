 ...
 *
 * Called before the vCPU releases vcpu->mutex and blocks, capturing the
 * register state into the ring slot so the agent has a complete snapshot
 * without an extra KVM_GET_REGS ioctl round-trip for the common case.
 * For exotic registers (FPU, XSAVE, debug regs), the agent can call
 * standard KVM ioctls on the duplicated vCPU fd since vcpu->mutex is
 ...
 * @resp: KVM_VMI_RESPONSE_* bitmask from userspace.
 *
 * IO uses deferred-instruction pattern (like CR/MSR):
 ...
 *
 * Called by the generic VMI event-control path for events with
 ...
 * per-session CPU state to restore synchronously on teardown. See kvm_vmi_release().
 */
 ...
 * Called from handle_monitor_trap() when VMI singlestepping is active.
 * Disables MTF (one-shot behavior) and delivers the singlestep event via
 * ring if singlestep monitoring is enabled.
 */
		     ...
		     u64 new_val)
{
	...
	/* bitmask filter (only active when onchangeonly is set): skip if changed bits do not overlap with mask */
 ...
 *         1 (handled - response handler controls emulation via flags).
 */
