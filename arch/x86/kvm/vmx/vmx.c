...
/**
 * vmx_vmi_update_msr_intercept - Enable/disable MSR write exiting for VMI
 * @vcpu: The vCPU to update.
 * @msr: The MSR index.
 * @enable: Whether to enable WRMSR exiting for this MSR.
 *
 ...
 * only update in-memory VMI state (kvm_vcpu_vmi fields); this function
...
void vmx_vmi_apply_vmcs_state(struct kvm_vcpu *vcpu)
{
	 ...
	 * supports it. vmx_tertiary_exec_control() checks kvm->vmi.
	 */
	if (cpu_has_tertiary_exec_ctrls()) {
		struct vcpu_vmx *vmx = to_vmx(vcpu);
